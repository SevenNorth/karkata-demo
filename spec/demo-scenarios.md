...
固定镜头的 14 x 9 格小农场，角色、农田、商店、回收屋、独立农舍和独立仓库同时可见，不滚动地图。农舍用于睡觉休整，仓库用于存取物品，商店支持购买种子和售卖作物，商店旁的回收屋也可售卖作物。用户通过右侧对话框发出自然语言指令，浏览器中的 Karkata Agent 选择农场工具，工具调用本地 `FarmSimulation` 完成状态变更，Phaser 只负责渲染和动画。
...
`get_farm_state.tiles` 是可操作农田坐标和格子状态的唯一模型上下文来源。格子状态使用明确的 `grass -> tilled -> planted -> watered` 生命周期。坐标型工具的描述与 Schema 不复制固定地图范围，只校验坐标为整数；模型根据最新状态选择格子，`FarmSimulation` 在执行时对坐标和状态做最终校验。

## 架构边界

- `FarmSimulation`：可序列化的游戏状态和规则。
- `farmTools.ts`：Zod 输入校验和 Agent 工具适配。
- `FarmCanvas.tsx`：固定相机 Phaser 场景，状态变化后重绘，并使用角色图集播放逐格行走动画。
