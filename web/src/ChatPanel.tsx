...
export function ChatPanel({ mode, authenticated, agentState, onSubmit, onAbort, onMinimize, onRestore, onTogglePlacement }: ChatPanelProps) {
  ...
  return <aside className={`chat-panel panel-${mode}`} style={mode === 'floating' ? { transform: `translate(${position.x}px, ${position.y}px)` } : undefined}>
    ...
    {!minimized && <><div className="message-list" ref={messageList} onScroll={(event) => { const element = event.currentTarget; followLatest.current = element.scrollHeight - element.scrollTop - element.clientHeight < 48 }}>{agentState.items.length === 0 && <div className="chat-empty"><Bot size={20} /><strong>告诉 Agent 你想做什么</strong><span>例如：帮我把左上角三块地耕好并种上防风草。</span><span>也可以说：查看农场状态、浇水、收获或睡觉。</span></div>}{agentState.items.map((item) => item.type === 'message' ? <div className={`message-row ${item.role}`} key={item.id}><div className="message-avatar">{item.role === 'assistant' ? <Bot size={14} /> : <UserRound size={14} />}</div><div className={`message-bubble ${item.contentStatus}`}>{item.content}</div></div> : <div className={`tool-row ${item.status}`} key={item.id}>工具 {item.name} · {item.status}</div>)}{agentState.error && <div className="agent-error">{agentState.error.message}</div>}</div>
