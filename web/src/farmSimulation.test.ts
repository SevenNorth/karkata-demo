...
describe('FarmSimulation', () => {
  ...
  it('moves items between the backpack and farmhouse storage', () => {
    const farm = new FarmSimulation()
    farm.moveTo('storage')
  ...
  it('requires the bed location before advancing the day', () => {
    const farm = new FarmSimulation()
    farm.moveTo('field')
    expect(() => farm.sleep()).toThrow('请先移动到床边')
    farm.moveTo('bed')
    expect(farm.sleep()).toMatchObject({ day: 2, energy: 100, location: 'bed' })
  })
})
