...
export type FarmTile = { x: number; y: number; state: 'grass' | 'tilled' | 'planted' | 'watered'; crop: SeedType | null; growth: number }
export type FarmPlace = 'field' | 'shop' | 'farmhouse' | 'storage' | 'recycling'
...
export class FarmSimulation {
  private state: FarmSnapshot = {
    day: 1, time: '06:00', energy: 100, gold: 100, player: { x: 8, y: 7 }, location: 'farmhouse',
    inventory: [{ item: 'parsnip_seed', quantity: 3 }], storage: [],
    tiles: Array.from({ length: FIELD_W * FIELD_H }, (_, index) => ({ x: FIELD_X + index % FIELD_W, y: FIELD_Y + Math.floor(index / FIELD_W), state: 'grass' as const, crop: null, growth: 0 })),
    message: '新的一天开始了。',
  }
  ...
  private placeName(place: FarmPlace) { return ({ field: '农田', shop: '商店', farmhouse: '农舍', storage: '仓库', recycling: '回收屋' })[place] }
  ...
  moveTo(target: FarmPlace) { const positions = { field: { x: 4, y: 3 }, shop: { x: 9, y: 2 }, recycling: { x: 12, y: 2 }, farmhouse: { x: 8, y: 7 }, storage: { x: 11, y: 7 } }; this.state.player = positions[target]; this.state.location = target; this.commit(`角色走到了${this.placeName(target)}。`); return this.snapshot() }
  tillSoil(x: number, y: number) { this.requirePlace('field'); const tile = this.tile(x, y); if (tile.state !== 'grass') throw new Error('这块地已经耕过了。'); this.spendEnergy(2); tile.state = 'tilled'; this.commit(`已耕地 (${x}, ${y})。`); return this.snapshot() }
  ...
  sellItems(item: 'parsnip', quantity: number) { this.requirePlace('shop', 'recycling'); this.remove(this.state.inventory, item, quantity); this.state.gold += quantity * 35; this.commit(`在${this.state.location === 'shop' ? '商店' : '回收屋'}卖出 ${quantity} 个防风草，获得 ${quantity * 35} 金币。`); return this.snapshot() }
  ...
  sleep() { this.requirePlace('farmhouse'); this.state.day += 1; this.state.time = '06:00'; this.state.energy = 100; for (const tile of this.state.tiles) { if (tile.state === 'planted') { tile.state = 'watered'; tile.growth = 1 } else if (tile.state === 'watered') tile.growth += 1 }; this.commit(`在农舍睡了一觉，现在是第 ${this.state.day} 天。`); return this.snapshot() }
}
