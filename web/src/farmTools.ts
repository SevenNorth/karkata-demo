...
export function createFarmTools(simulation: FarmSimulation): Tool[] {
  ...
  const move: Tool<{ target: FarmPlace }, FarmSnapshot> = { name: 'move_to', description: 'Move the farmer to the field, shop, bed, storage chest, or shipping bin before performing a local action.', inputSchema: z.object({ target: z.enum(['field', 'shop', 'bed', 'storage', 'shipping_bin']) }), execute: ({ target }) => simulation.moveTo(target) }
  ...
  const sell: Tool<{ item: 'parsnip'; quantity: number }, FarmSnapshot> = { name: 'sell_items', description: 'Sell harvested parsnips for 35 gold each at the shop or shipping bin.', inputSchema: z.object({ item: z.enum(['parsnip']), quantity: z.number().int().min(1).max(20) }), execute: ({ item, quantity }) => simulation.sellItems(item, quantity) }
  const store: Tool<{ item: string; quantity: number }, FarmSnapshot> = { name: 'store_items', description: 'Move items from the backpack into the farmhouse storage chest.', inputSchema: z.object({ item: z.enum(['parsnip_seed', 'parsnip']), quantity: z.number().int().min(1).max(20) }), execute: ({ item, quantity }) => simulation.storeItems(item, quantity) }
  const withdraw: Tool<{ item: string; quantity: number }, FarmSnapshot> = { name: 'withdraw_items', description: 'Move items from the farmhouse storage chest into the backpack.', inputSchema: z.object({ item: z.enum(['parsnip_seed', 'parsnip']), quantity: z.number().int().min(1).max(20) }), execute: ({ item, quantity }) => simulation.withdrawItems(item, quantity) }
  const sleep: Tool<Empty, FarmSnapshot> = { name: 'sleep', description: 'Sleep while standing beside the farmhouse bed to restore energy and advance one day.', inputSchema: z.object({}), execute: () => simulation.sleep() }
