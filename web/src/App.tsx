...
export function App() {
  ...
  const logout = async () => { agentStore.abort(); browserAgent.clearHistory(); await fetch('/auth/logout', { method: 'POST', credentials: 'include' }); setAuth({ authenticated: false, user: null }) }
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions"><span className="status-dot" /> Proxy offline scaffold {auth.user && <button className="user-chip" title="退出 GitHub 登录" onClick={logout}>@{auth.user.login}</button>}</div>
      </header>
      <section className={`workspace panel-${panelMode}`}>
        ...
        <ChatPanel mode={panelMode} authenticated={auth.authenticated} agentState={agentState} onSubmit={(input) => agentStore.submit(input)} onAbort={() => agentStore.abort()} onMinimize={minimize} onRestore={restore} onTogglePlacement={togglePlacement} />
      </section>
    </main>
  )
}
